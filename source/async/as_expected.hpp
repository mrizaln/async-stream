...
namespace async
{
    ...
    struct AsExpected
    {
         ...
         * constructible. the underlying completion token is itself defaulted as an argument to allow it to capture a
         * source location.
         */
...
// NOTE: any partial specialization here are for integration with Asio, don't change the name of it. some typedef also
// for integration and have a fixed name
namespace asio
{

    template <typename T>
    using ExpectedHandler = ::async::detail::ExpectedHandler<T>;

    template <typename CompletionToken, typename Signature>
    class async_result<async::AsExpected<CompletionToken>, Signature>
    {
    private:
        using ExpectedSignature = ::async::detail::ExpectedSignatureTrait_t<Signature>;
        using ReturnType        = typename async_result<CompletionToken, ExpectedSignature>::return_type;

    public:
        template <typename Initiation, typename... Args>
        static ReturnType initiate(Initiation&& initiation, async::AsExpected<CompletionToken>&& token, Args&&... args)
        {
